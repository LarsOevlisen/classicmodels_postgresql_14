...
-- Solution assumes that a productcode can only appear once for each order (within rows with the same ordernumber). This is verified by below query. 
SELECT
	*
FROM (
	SELECT
	...
	FROM classicmodels.orderdetails) AS t1
WHERE t1.row_num > 1;

-- Exercise solution
WITH count_orders_with_product AS (
	-- For each productcode, in how many orders does the product appear
	SELECT
		productcode,
		COUNT(ordernumber) AS n_orders_with_product
	FROM classicmodels.orderdetails
